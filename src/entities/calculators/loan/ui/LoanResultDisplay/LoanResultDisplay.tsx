    ...
    >
      <View className="flex-row items-center justify-between">
        <Text className={`text-sm font-medium ${hasData ? accentColor : 'text-slate-400'}`}>
          {isDiff ? 'Первый и последний платеж' : 'Ежемесячный платеж'}
        </Text>
      ...
      <View style={{ minHeight: 48 }} className="justify-center">
        <View className="flex-row flex-wrap items-baseline gap-x-2 gap-y-1">
          <Text
            className={`text-4xl font-bold tracking-tight ${hasData ? 'text-slate-900' : 'text-slate-300'}`}
          >
            {formatMoney(firstPayment)}
          </Text>

          {isDiff && hasData && (
            <Text className="text-base font-medium text-slate-900 opacity-60">
